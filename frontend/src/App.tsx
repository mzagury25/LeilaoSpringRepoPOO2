...
const initialAuctions = [
  {
    ...
    sellerInfo: { id: 1, name: 'Galeria de Arte Moderna', avatar: 'https://i.imgur.com/gA3CfA6.jpeg', memberSince: 'Vendedor desde 2020' },
  },
];
...
const AuctionProvider = ({ children }) => {
    ...
    return (
        <AuctionContext.Provider value={{ auctions, getAuctionById, addAuction, placeBid, watchlist, toggleWatchlist, categories: initialCategories }}>
            {children}
...
const HomePage = () => {
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-6">Leilões em Destaque</h1>
                    ...
                    <a href={`#/produto/${auction.id}`} key={auction.id} className="group block">
                        <Card className="overflow-hidden h-full flex flex-col">
                            ...
                            <CardContent className="mt-auto">
                                ...
                                <div className="text-xl font-bold">R$ {auction.auctionInfo.currentBid.toFixed(2)}</div>
                            </CardContent>
                        </Card>
                    </a>
                ))}
...
const HowItWorksPage = () => (
    <div className="max-w-4xl mx-auto prose prose-invert">
        ...
        <ol>
            <li><strong>Cadastre-se:</strong> Crie sua conta gratuitamente em nosso site para começar a participar dos leilões.</li>
            <li><strong>Explore:</strong> Navegue por nossas categorias e encontre itens de seu interesse. Use a busca para encontrar algo específico.</li>
            <li><strong>Dê seu Lance:</strong> Encontrou algo que gostou? Digite o valor do seu lance e confirme. Você pode usar os botões de incremento para facilitar.</li>
            <li><strong>Acompanhe:</strong> Adicione itens à sua "Lista de Observação" para não perdê-los de vista.</li>
            <li><strong>Arremate:</strong> Se o seu lance for o maior ao final do leilão, parabéns! O item é seu. Você receberá uma notificação para prosseguir com o pagamento.</li>
        </ol>

        <h2>Para Vendedores</h2>
        <ol>
            <li><strong>Anuncie seu Item:</strong> Vá em "Anunciar um Item" no menu do seu perfil. Preencha todos os detalhes, como título, descrição, lance inicial e uma imagem de qualidade.</li>
            <li><strong>Acompanhe o Leilão:</strong> Veja os lances sendo feitos em tempo real na página do seu produto.</li>
            <li><strong>Venda Concluída:</strong> Ao final do leilão, o maior lance arremata o item. Nós cuidamos da notificação e do processo de pagamento.</li>
            <li><strong>Receba seu Pagamento:</strong> Após a confirmação do pagamento pelo comprador, o valor será transferido para você com segurança.</li>
        </ol>
    </div>
);
...
const AnnounceItemPage = () => {
    ...
    const handleSubmit = (e) => {
        ...
        const newAuctionData = {
            ...
            details: { // <-- FIX: Adiciona um objeto de detalhes padrão
                Categoria: categories.find(c => c.id === category)?.name || 'Não especificado',
                Condição: 'Não especificado',
            },
            auctionInfo: {
                        ...
                        <a href={`#/produto/${item.id}`} key={item.id} className="block">
                            <Card className="p-4 flex items-center gap-4 hover:bg-slate-800 transition-colors">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="font-bold text-lg">R$ {item.auctionInfo.currentBid.toFixed(2)}</p>
                                </div>
                            </Card>
                        </a>
                    ))
...
const ImageGallerySection = ({ images, title }) => (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        {images.map((src, index) => (
            <img key={index} src={src} alt={`${title} - imagem ${index + 1}`} className="w-full h-auto object-cover rounded-lg shadow-lg" />
        ))}
    </div>
);

const AuctionDetailsSection = ({ auctionInfo, endDate, onBid, isWatched, onToggleWatchlist }) => {
    ...
    const handleBidSubmit = (e) => {
        ...
        if (bidAmount < minBid) {
            alert(`O lance mínimo é de R$ ${minBid.toFixed(2)}`);
            return;
        }
        onBid(bidAmount);
    };
    ...
    return (
        <Card className="p-6 mb-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6 text-center">
                <div>
                    <p className="text-sm text-slate-400">Lance Atual</p>
                    <p className="text-2xl font-bold">R$ {currentBid.toFixed(2)}</p>
                </div>
                <div>
                    <p className="text-sm text-slate-400">Lance Mínimo</p>
                    <p className="text-2xl font-bold">R$ {minBid.toFixed(2)}</p>
                </div>
                 <div>
                    <p className="text-sm text-slate-400">Incremento</p>
                    <p className="text-2xl font-bold">R$ {increment.toFixed(2)}</p>
                </div>
            </div>
            ...
            {!countdown.isFinished && (
                <form onSubmit={handleBidSubmit} className="flex flex-col items-center gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <Button type="submit" variant="primary" size="lg">Fazer Lance</Button>
                        <Button type="button" variant={isWatched ? 'destructive' : 'secondary'} size="lg" onClick={onToggleWatchlist}>
                            <HeartIcon className="mr-2" /> {isWatched ? 'Remover da Lista' : 'Adicionar à Lista'}
                        </Button>
                    </div>
                </form>
            )}
...
const ItemDetailsSection = ({ details }) => {
    // FIX: Adiciona uma verificação para garantir que 'details' não é nulo ou indefinido
    if (!details || Object.keys(details).length === 0) {
        return null; // Não renderiza nada se não houver detalhes
    }
                ...
                <TableBody>
                    {history.map((bid, index) => (
                        <TableRow key={index}>
                            ...
                            <TableCell className="text-right">R$ {bid.amount.toFixed(2)}</TableCell>
...
const App = () => {
    ...
    switch (path[1]) {
        case 'produto':
            PageComponent = <ProductPage routeId={path[2]} />;
            break;
        case 'categorias':
